...
parser.add_argument('--num-epochs', type=int, default=1000, help='number of epochs for training')
...
def load_data(dataset_dir, batch_size, num_workers):
    if args.evaluation:
        ...
        test_dataset_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
        return test_dataset_loader, len(test_dataset)
    else:
        train_val_dataset = {
            tag: datasets.ImageFolder(root=os.path.join(dataset_dir, tag), transform=transform)
            for tag in ['train', 'val']
        }
        train_val_dataset_size = {tag: len(train_val_dataset[tag]) for tag in ['train', 'val']}
        train_val_dataset_loader = {
            tag: DataLoader(train_val_dataset[tag], batch_size=batch_size, shuffle=True, num_workers=num_workers)
            for tag in ['train', 'val']
        }
        return train_val_dataset_loader, train_val_dataset_size


def save_model(model, tag, stream='infrared'):
    if tag is 'top':
...
def train(model, dataloader, num_epochs, dataset_size, device):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            print('  {0} Loss: {1:.4f} Acc: {2:.4f}'.format(phase, epoch_loss[phase], epoch_accuracy[phase]))

        if epoch_accuracy['val'] > top_accuracy:
            print('best model ever! save at global step {0}'.format(epoch))
...
def main():
    ...
    dataset_dir = '../' + args.dataset_dir + '/Data/Infrared/'
