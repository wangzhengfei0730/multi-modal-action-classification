...
video_dir = '../PKUMMDv1/Data/DEPTH_VIDEO'
label_dir = '../PKUMMDv1/Label'
depth_dir = '../PKUMMDv1/Data/Depth'
...
def preprocess_video(data_id):
    try:
        ...
        video_path = os.path.join(video_dir, data_id + '-depth.avi')
        ...
        for i in range(num_actions):
            ...
            cur_depth_dir = os.path.join(depth_dir, '{:02}'.format(cur_action_class))
            if not os.path.exists(cur_depth_dir):
                os.mkdir(cur_depth_dir)

            start_frame, end_frame = start_frames[i], end_frames[i]
            frame_interval = max(1, (end_frame - start_frame) // NUM_FRAME_SAMPLE)
            frame_count = 0

            while ret and frame_id < start_frame:
                ret, frame = video_capture.read()
                frame_id += 1
            
            cur_frame_id = 0
            while ret and frame_id < end_frame:
                if (cur_frame_id + 1) % frame_interval == 0:
                    depth_filename = data_id + '-depth-{:02}.jpg'.format(frame_count)
                    depth_filename_path = os.path.join(cur_depth_dir, depth_filename)
                    cv2.imwrite(depth_filename_path, frame)
                    frame_count += 1
